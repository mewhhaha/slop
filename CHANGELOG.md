...
* Breaking: public failures now use the unprefixed `Error`, `Result`, `renderError`, and `throwError` API; their constructors retain SDL, asset, shader, binding, and file evidence.
* Breaking: the global fragment uniform API is now `Globals` and `globals`, with no redundant package prefix.
